...
**Status:** v0.1 draft.
**Audience:** anyone who wants to understand what rrvix is in twenty minutes.
**Reading order:** start here. Then [`0002-cir.md`](0002-cir.md) if you want the data model, [`0003-claim-graph.md`](0003-claim-graph.md) for how knowledge composes, [`0004-tex-template.md`](0004-tex-template.md) for how authors write rrvix papers.

## What rrvix is

rrvix is an open protocol for research preprints. It pairs the immutability and citability of arXiv with a Wikipedia-style commentary layer, plus a structured **claim graph** that makes the corpus natively queryable by software agents.
...
- **Not a chat interface to papers.** Read access via the API is symmetric for humans and agents, but rrvix doesn't itself host conversational agents. It's a protocol for a structured corpus that any agent can read.
...
The whitepaper makes the case for why this is worth doing now. This document is the spec-level summary of what rrvix actually *is*.

## How rrvix is shaped

### Papers

A **paper** in rrvix is an immutable submission. It has:
...
- An optional list of topic IDs from the rrvix controlled vocabulary.

Papers can be authored in any compile-friendly format (LaTeX is the v0.1 reference; Typst is on the v0.2 roadmap). The reference LaTeX class — [`rrvix.cls`](../template/rrvix.cls) — provides semantic environments that the parser extracts into structured form.

### Claims

A **claim** is a falsifiable assertion extracted from a paper. Claims are the unit of structured knowledge. Each claim has:

- A stable, globally unique ID. The convention is `<paper_id>:<local_label>` (e.g. `rrvix-0001:claim:queryability`), but any globally unique string is valid.
...
These queries are first-class in the rrvix API. They are not feasible against an unstructured PDF corpus, with or without LLM mediation.
...
The **Canonical Intermediate Representation (CIR)** is the agent-readable form of a paper. Every paper in rrvix has exactly one canonical CIR per submitted revision. The CIR is produced by the rrvix parser from the paper's plain-text source plus a sidecar metadata file that the LaTeX class emits during compilation.

The flow:

1. Author writes the paper in `rrvix.cls` LaTeX. They use `\begin{claim}` and similar environments for the structured pieces.
2. On compile, `rrvix.cls` writes a `<basename>.rrvix.aux` sidecar containing structured markers — paper metadata, sequential markers for each claim/evidence/observation/scope/openquestion/remark block, and inline edge declarations.
3. The rrvix parser (reference implementation: `rrvix-python`) reads the source plus the sidecar and produces a CIR JSON object.
...
Citation has to work over decades. The moment any paper revision can mutate, every downstream citation graph becomes lying. rrvix preserves citation integrity by making submitted papers (and their claims) immutable. Errors get errata; revisions get new versions with explicit version chains. Nothing ever silently changes.

### Why claims, not full-text search

Full-text search returns a paper. The reader still has to identify which claim in the paper is the relevant one. rrvix returns *the claim* — with its scope, evidence, replications, contradictions, and extensions inline. The unit of retrieval matches the unit of reasoning.
...
A protocol whose canonical implementation can be captured is not a public good. Mandatory free snapshot exports plus CC-BY licensing on submitted content make the corpus genuinely fork-able. If rrvix.org is ever run badly, the community can mirror the corpus and run a competing instance the next day. This is the same property that protects DNS and email from any single operator.

### Why agents are first-class

Agents *are* heavy producers and consumers of research today. Pretending otherwise produces incoherent design choices — pricing tiers, rate limits, "no scraping" terms — that punish exactly the use case the corpus is most useful for. rrvix's stance is that read access is free and unlimited (modulo abuse mitigation), write access is symmetric (agents can submit annotations, claim extractions, even papers, with the `is_agent` flag), and the API is shaped for agent throughput rather than browser interaction.
...
| [0004](0004-tex-template.md) | `rrvix.cls` reference |
...
The whitepaper ([`whitepaper/rrvix-whitepaper.tex`](../whitepaper/rrvix-whitepaper.tex)) is the authoritative narrative of why this is built. The schemas under [`schema/`](../schema/) are the authoritative data model. This spec sits between them: prose-level, but precise about what's in scope for v0.

## Status

v0.1. This document and the spec series are live drafts. The whitepaper is at v0.1; CIR schema is v0.1.0; `rrvix.cls` is v0.1. None of these are stable yet — the whole point of the v0.x series is to find the rough edges before anyone builds a serious instance.
