...
The **CIR** is the structured representation of a rrxiv paper. Every paper in rrxiv has exactly one canonical CIR per submitted revision. It is the unit servers store, the unit clients read, the unit agents query. The plain-text source is preserved and served too — but the CIR is what gets indexed, edge-walked, and summarised.

A CIR is JSON. It is produced by the rrxiv parser from a paper's source plus the sidecar metadata file that `rrxiv.cls` emits during compilation. The CIR does not contain the rendered PDF; the `source.rendered_pdf_uri` field links to it.
...
├── citations[]   — the bibliography, with optional rrxiv paper IDs for cited works
...
The required fields are: `rrxiv_version`, `id`, `version`, `title`, `authors`, `abstract`, `submitted_at`, `license`, `source`. Everything else is optional.

| Field | Type | Notes |
|-------|------|-------|
| `rrxiv_version` | string (SemVer) | Protocol version this CIR conforms to. v0 papers must use `0.x.y`. |
| `id` | string | Stable, **opaque** machine identifier (server-assigned UUIDv7). Once assigned, never changes; clients MUST NOT parse it — cite `id_slug` instead. See RRP-0029. |
| `id_slug` | string (optional) | Human-facing citable id `rrxiv:YYMM.NNNNN` (RRP-0013). Server-minted, per-instance; used in URLs and citations. Servers mint one at ingest. |
| `version` | string (`v\d+`) | Paper revision (`v1`, `v2`, …). Each version is itself immutable. |
| `previous_version` | string \| null | The prior revision's `id`, if any. Null for `v1`. |
...
| `topics` | array of strings | Topic IDs from the rrxiv controlled vocabulary. May be amended post-submission via annotations. |
...
{
  "format": "latex",
  "uri": "https://rrxiv.com/papers/<id>/<version>/source.tar.gz",
  "rendered_pdf_uri": "https://rrxiv.com/papers/<id>/<version>/paper.pdf",
...
# Python / pydantic (used by rrxiv-python)
...
When the schema bumps, the change goes through an RRP that documents the migration. Old CIRs are not rewritten; they continue to declare their original `rrxiv_version`. Tools must accept any v0.x CIR for read; they should reject `rrxiv_version` outside their supported range for write paths.
...
{
  "rrxiv_version": "0.1.0",
  "id": "rrxiv-example-minimal",
  "version": "v1",
  "title": "A minimal rrxiv paper",
  "authors": [{ "name": "rrxiv Project" }],
  ...
  "claims": [
    {
      "id": "rrxiv-example-minimal:claim:fixture",
      "paper_id": "rrxiv-example-minimal",
      "statement": "A minimal rrxiv paper produces a valid CIR ...",
  ...
  "citations": [
    {
      "id": "cite-rrxiv-example-minimal:rrxiv-cir-schema",
      "key": "rrxiv-cir-schema",
      "bibtex_entry": "@misc{rrxiv-cir-schema, ...}"
    }
  ],
  "annotations": []
}
...
This is the actual CIR produced by `rrxiv-python` from `template/examples/minimal/` — see `tests/schemas/fixtures/cir-valid-minimal.json` for a more elaborate version that exercises the full schema.
